...
def get_attn_layer(embedding_seq,num_cells=64, input_len=20,layer_name=None):
    ...
    if layer_name:
        att = Attn(num_cells,name=layer_name)(td_dense)
    else:
    ...
    print("softmax shape:",tf.keras.backend.int_shape(word_att_softmax))
    #word_att_softmax_reshape = tf.keras.layers.Reshape((input_len,1))(word_att_softmax)
    word_att_softmax_reshape = tf.keras.layers.Lambda(lambda x: tf.keras.backend.reshape(x,shape=(input_len,1)))(word_att_softmax)
...
def get_model(vocab_size=1000,emb_dim = 100, input_len = (4,20), output_size = 2, num_cells = 64):
    ...
    for sequence_input in sequence_inputs:
        ...
        attn_out = get_attn_layer(num_cells=num_cells,input_len=input_len[1],embedding_seq=embedding_seq,layer_name="attention_"+str(i))
